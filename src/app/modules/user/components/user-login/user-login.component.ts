  ...
  onSubmit(order: string){
    ...
    if(order === 'submit'){
      ...
      if(localStorage.getItem('user')){
        ...
        if(this.user.username === userData.username && this.user.mail === userData.mail && userData.admin === 0){
        ...
        if(this.user.username === userData.name && this.user.mail === userData.mail && userData.admin === false){
          userData.admin = true;
          userData.state = "connected"
          localStorage.setItem('user', JSON.stringify(userData))
          this.userService.refresh.emit(2)
